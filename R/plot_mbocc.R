  ...
  for (plot in c("MLE","Site","State")){
    ...
    if (plot=="MLE"){
      ...
      for (i in 1:nrow(plots)){
        ...
        if (return==T){
          p1 <- plots[i,1]; p1 <- gsub(" ",".",p1,fixed=T)
          p2 <- plots[i,2]; if (p2!="~1"){p2 <- gsub("~","",p2,fixed=T)}
          out[[paste(p1, p2)]][["MLE"]] <- g
        } else {print(g)}
      }
    }
    ...
    if (plot=="Site"){
      ...
      for (i in 1:nrow(nplots)){
        ...
        for (j in groups){
          ...
          if (any(!is.na(pull))){
            ...
            if (return==T){
              out[[paste(nplots[i,2],nplots[i,3])]][[j]][["Site"]] <- g1
            } else {print(g1)}
          }
        }
      }
      if (any(nplots$call==1)){
        ...
        for (i in unique(pull$psi)){
          ...
          if (return==T){out[[paste(i,"~1")]][["Site"]] <- g2} else {return(g2)}
        }
      }
    }
    ...
    if (plot=="State"){
      ...
      if (any(plots$call==1)){
        ...
        for (i in unique(pull$psi)){
          ...
          if (return==T){out[[paste(i,"~1")]][["State"]] <- g2} else {return(g2)}
        }
      }
    }
  }
